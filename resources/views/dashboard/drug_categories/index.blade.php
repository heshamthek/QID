...
<div class="w-full max-w-7xl mx-auto mt-12 p-6 bg-white rounded-lg shadow-lg">
    ...
    <div class="overflow-x-auto" id="drugCategoriesTable">
        <table class="min-w-full leading-normal">
            <thead>
                <tr>
                    <th class="px-5 py-3 border-b-2 border-gray-300 bg-gray-200 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Category Name</th>
                    <th class="px-5 py-3 border-b-2 border-gray-300 bg-gray-200 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Actions</th>
                </tr>
            </thead>
            <tbody>
                @foreach($drugCategories as $drugCategory)
                <tr class="hover:bg-gray-100 transition duration-200">
                    <td class="px-5 py-5 border-b border-gray-300 bg-white text-sm">{{ $drugCategory->category_name }}</td>
                    <td class="px-5 py-5 border-b border-gray-300 bg-white text-sm flex space-x-2">
                        <a href="{{ route('dashboard.drug_categories.edit', $drugCategory->id) }}" class="text-blue-600 hover:text-blue-700">
                            <i class="fas fa-edit"></i>
                        </a>
                        <button type="button" class="text-red-600 hover:text-red-700" onclick="openDeleteModal({{ $drugCategory->id }})">
